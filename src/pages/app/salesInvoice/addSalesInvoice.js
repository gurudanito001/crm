...
const AddSalesInvoice = () => {
  ...
  useEffect(()=>{
    if(formData.invoiceRequestId){
      let data =  getCustomerAndEmployeeId(formData.invoiceRequestId)
      setFormData( prevState =>({
        ...prevState,
        ...data
      }))
    }
  }, [formData.invoiceRequestId])

  const queryClient = useQueryClient();
  const salesInvoiceMutation = useMutation({
    mutationFn: ()=> apiPost({ url: `/salesInvoice/create`, data: formData })
    .then(res => {
      dispatch(
        setMessage({
          severity: "success",
  ...
  return (
    <Layout>
      <section className="px-3 py-5 p-lg-5" style={{ maxWidth: "700px" }}>
        <header className="h3 fw-bold">Add Sales Invoice </header>
        <p>Fill in Sales Invoice Information.</p>
          <form className="mt-5">
          <div className="mb-3">
