...
const AddMonthlyVisitPlan = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const queryClient = useQueryClient();
  const monthlyVisitPlanMutation = useMutation({
    mutationFn: ()=> apiPost({ url: `/monthlyVisitPlan/create`, data: formData }).then(res => {
      dispatch(
        setMessage({
          severity: "success",
          message: res.message,
  ...
  const handleSubmit = (e) =>{
    ...
    monthlyVisitPlanMutation.mutate();
  }
      ...
      <section className="px-3 py-5 p-lg-5" style={{ maxWidth: "700px" }}>
        ...
        <form className="mt-5">
          ...
          <div className="d-flex mt-5">
            <button className="btn btnPurple m-0 px-5" disabled={monthlyVisitPlanMutation.isLoading} onClick={handleSubmit}>{monthlyVisitPlanMutation.isLoading ? <Spinner /> : "Submit"}</button>
...
export default AddMonthlyVisitPlan;