...
const AllVisitPlansAdmin = () => {
  ...
  const weeklyVisitPlanQuery = useQuery({
    ...
    queryFn: () => apiGet({url: "/weeklyVisitPlan"})
  ...
  const monthlyVisitPlanQuery = useQuery({
    ...
    queryFn: () => apiGet({url: "/monthlyVisitPlan"})
    .then( (res) => res.payload)
    .catch( error =>{
      dispatch(
        setMessage({
  ...
  const listSortedVisitPlanData = () =>{
    let sortedVisitPlans = sortVisitPlans(currentTab);

    let keys = Object.keys(sortedVisitPlans);
    return keys.map( key => {
  ...
  return (
    <Layout>
      <section className="px-3 py-5 p-lg-5" style={{ maxWidth: "700px" }}>
        ...
        <ul className='mt-5'>
        {(!weeklyVisitPlanQuery.isLoading || monthlyVisitPlanQuery.isLoading) && (!weeklyVisitPlanQuery.isError || monthlyVisitPlanQuery.isError) && (listSortedVisitPlanData())}
...
export default AllVisitPlansAdmin;
